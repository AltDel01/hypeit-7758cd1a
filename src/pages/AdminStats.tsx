...
import { RefreshCw, BarChart3, Users, CheckCircle, Clock, TrendingUp } from 'lucide-react';
...
const AdminStats = () => {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      setTotalStats({ total, completed, inProgress, newCount });
      ...
      const stats: EditorStat[] = editorIds.map((uid) => {
        ...
        return { name, completed: completedReqs.length, inProgress: inProgressReqs.length, avgHours };
      });
        ...
        <main className="flex-1 p-4 md:p-8">
          <div className="max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h1 className="text-3xl font-bold mb-2 text-foreground">Performance Stats</h1>
                <p className="text-muted-foreground">Track editor performance and request metrics</p>
              </div>
              <Button onClick={() => { loadStats(); toast.info('Refreshed'); }} variant="outline" className="gap-2">
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  {/* Editor Completions Bar Chart */}
                  {editorStats.length > 0 && (
                    <div className="bg-card/50 backdrop-blur-sm border border-border rounded-lg p-4">
                      <h2 className="font-semibold text-foreground mb-4">Completions by Editor</h2>
                      <ResponsiveContainer width="100%" height={250}>
                ...
                {editorStats.length > 0 && (
                  <div className="bg-card/50 backdrop-blur-sm border border-border rounded-lg overflow-hidden">
                    <h2 className="font-semibold text-foreground p-4 pb-2">Editor Speed Rankings</h2>
                          ...
                          .map((editor, i) => (
                            <tr key={editor.name} className="border-b border-border/50 hover:bg-muted/20">
                              ...
                              <td className="p-3 text-center text-foreground">
                                {editor.avgHours === null
                                  ? '—'
                                  : editor.avgHours < 1
                                  ? `${Math.round(editor.avgHours * 60)}m`
                                  : `${editor.avgHours.toFixed(1)}h`}
                              </td>
                            </tr>
                          ))}
                      </tbody>
