    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8">
        <div>
          ...
          <p className="text-brand-slate-600">Welcome back! Here's an overview of your BrandGen activity.</p>
        </div>
        <div className="mt-4 md:mt-0 flex space-x-3">
          <Button variant="outline" className="border-brand-slate-200">
            <Clock className="mr-2 h-4 w-4" />
            View History
          </Button>
          <Button className="bg-gradient-to-r from-brand-blue to-brand-teal text-white hover:shadow-md">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {usageMetrics.map((metric, idx) => (
            <GlassMorphismCard key={idx} className="p-4">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="font-medium text-brand-slate-800">{metric.label}</h3>
                  <div className="mt-1">
        ...
        <GlassMorphismCard className="overflow-hidden">
          <div className="divide-y divide-brand-slate-100">
            {recentActivity.map((activity, idx) => (
              <div key={idx} className="flex items-center p-4 hover:bg-brand-slate-50 transition-colors">
                <div className="mr-4">
                  {activity.type === "image" || activity.type === "moodboard" ? (
                    ...
                    <div className="h-12 w-12 flex items-center justify-center rounded-md bg-brand-slate-100 text-brand-slate-500">
                      <MessageSquare className="h-5 w-5" />
                    </div>
                  )}
                </div>
                <div className="flex-1">
                  <h3 className="font-medium text-brand-slate-800">{activity.title}</h3>
                  <p className="text-sm text-brand-slate-500">{activity.time}</p>
                </div>
                <Button variant="ghost" size="sm" className="text-brand-slate-500">
                  View
                </Button>
              </div>
            ))}
...
const ActionCard = ({
  ...
  return (
    <GlassMorphismCard 
      className={cn(
        "p-4 hover:cursor-pointer transition-all duration-300",
        disabled ? "opacity-80" : "hover:-translate-y-1"
      )}
      onClick={disabled ? undefined : onClick}
    >
      <div className="flex items-start">
        <div className="flex items-center justify-center rounded-md p-2 mr-3 bg-brand-slate-100">
          {icon}
        </div>
        <div>
          <div className="flex items-center">
            <h3 className="font-medium text-brand-slate-800">{title}</h3>
            {upgradeBadge && (
          ...
          <p className="text-sm text-brand-slate-600">{description}</p>
        </div>
      </div>
    </GlassMorphismCard>
  );
