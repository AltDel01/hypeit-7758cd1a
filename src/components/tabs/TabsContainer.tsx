...
const TabsContainer = ({
  ...
  const renderContent = () => {
    if (activeTab === 'feed' || activeTab === 'story') {
            ...
            <div className="flex gap-2 mb-6">
              {[1, 3, 15, 25].map((count, index) => (
                <button
                  ...
                  className={`rounded-full px-5 py-2 ${selectedImagesPerBatch === count ? 'bg-gray-600' : 'bg-gray-800'} ${index > 1 ? 'flex items-center' : ''}`}
                >
                  <span>{count}</span>
                  {index > 1 && (
      ...
      return (
        <div className="rounded-md border border-gray-700 p-4 bg-gray-900 mt-6">
    ...
    <div className="w-full">
      <div className="grid grid-cols-2 gap-4 mb-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-64 bg-gray-800 border border-gray-700">
            <div className="p-3">
              {renderContent()}
            </div>
          </DropdownMenuContent>
