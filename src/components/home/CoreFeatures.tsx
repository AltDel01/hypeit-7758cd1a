  ...
  return <section className="relative py-12 md:py-24 px-3 md:px-4 overflow-hidden bg-black">
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 md:mb-20">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto text-xs md:text-base px-2">Create professional-quality videos in minutes, not hours.
Our AI handles the editing so you can focus on creating.</p>
        </div>
        {features.map((feature, index) => <div key={feature.id} className={`flex flex-col ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center gap-6 md:gap-8 lg:gap-16 mb-12 md:mb-24 last:mb-0`}>
            ...
            <div className="flex-1 w-full max-w-xl px-2 md:px-0">
              <div className="relative rounded-xl md:rounded-2xl overflow-hidden border border-white/10 bg-gray-900/50">
                {'video' in feature && feature.video ? <video src={feature.video} autoPlay loop muted playsInline className="w-full h-48 sm:h-56 md:h-80 object-cover" /> : <img src={feature.image} alt={feature.altText} className="w-full h-48 sm:h-56 md:h-80 object-cover" />}
